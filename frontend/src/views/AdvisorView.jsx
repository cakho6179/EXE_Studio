...
export default function AdvisorView() {
  ...
  // Chỉ liệt kê TÀI LIỆU THẬT user đã nạp — không bịa trích dẫn học thuật.
  const citations = useMemo(() => {
    if (!docs || docs.length === 0) return [];
    return docs.slice(0, 4).map((d, idx) => ({
  ...
  function exportBibtex() {
    ...
    // Chỉ xuất entry từ TÀI LIỆU THẬT đã nạp — không chèn trích dẫn mẫu cứng.
    if (docs.length === 0) {
      showToast('Chưa có tài liệu nào để xuất. Hãy nạp giáo trình/rubric trước.', 'warning');
      return;
    }
  ...
  function downloadObsidianNote() {
    ...
    const convoMd = messages.length > 0
      ...
      : `## Phiên trống\n\nChưa có nội dung thảo luận trong phiên này. Hãy đặt câu hỏi để bắt đầu.`;
    ...
    <div className="max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 pt-5 pb-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                      ...
                      <button type="button" onClick={() => setSessionId(s.id)} className="flex-1 min-w-0 text-left">
                        {editingSessionId === s.id ? (
                          <input
                            ...
                            onKeyDown={(e) => {
                              if (e.key === 'Enter') saveSessionRename(s.id);
                              else if (e.key === 'Escape') setEditingSessionId(null);
                            }}
                            onBlur={() => saveSessionRename(s.id)}
                            maxLength={120}
                            aria-label="Tên phiên mới"
                            className="w-full px-2 py-1 rounded-lg border border-blue-300 text-xs text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-200"
                          />
                        ) : (
                          <>
        ...
        <div className="lg:col-span-3 space-y-4">
          ...
          <div className="glass-card rounded-2xl p-4">
            <div className="flex items-center justify-between mb-3">
              <span className="text-xs font-bold uppercase tracking-wider text-slate-500">Tài liệu tham khảo đã nạp</span>
              <span className="px-2 py-0.5 text-[10px] bg-blue-50 text-blue-700 font-semibold rounded border border-blue-200">Nguồn thật</span>
            </div>
            <div className="space-y-2 text-xs">
              {citations.length === 0 && (
                <p className="text-[11px] text-slate-500 leading-relaxed">Chưa có tài liệu nào. Nạp giáo trình/rubric để AI trích dẫn đúng nguồn của bạn — không tự bịa trích dẫn.</p>
              )}
              {citations.map((c) => (
                <div key={c.tag} className="p-2.5 rounded-xl bg-slate-50 border border-slate-200/80">
                  <div className="flex items-center justify-between text-[11px] font-bold text-slate-800">
                    <span>{c.tag}</span>
                    <span className="text-blue-600 font-semibold">✔ Đã nạp</span>
                  </div>
